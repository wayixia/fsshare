...
void ShareRTCControl::onConnected() {
  signal_connection_.IdentifySelf();
}

void ShareRTCControl::onDataReceived( const uint8_t* data, size_t len ) {
  if( !data || len == 0 )
  {
...
int ShareRTCControl::SendMessage(const std::string& json_object) {

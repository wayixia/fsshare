...
ShareRTCSession::ShareRTCSession(const webrtc::Environment& env,
...
ShareRTCSession::~ShareRTCSession() {
  RTC_DCHECK(!peer_connection_);
}

bool ShareRTCSession::connection_active() const {
  return peer_connection_ != nullptr;
}

void ShareRTCSession::Close() {
  client_->SignOut();
  DeletePeerConnection();
}

bool ShareRTCSession::InitializePeerConnection() {
...
bool ShareRTCSession::ReinitializePeerConnectionForLoopback() {
  loopback_ = true;
...
bool ShareRTCSession::CreatePeerConnection() {
  RTC_DCHECK(peer_connection_factory_);
  RTC_DCHECK(!peer_connection_);
...
void ShareRTCSession::DeletePeerConnection() {
...
void ShareRTCSession::EnsureStreamingUI() {
  RTC_DCHECK(peer_connection_);
...
void ShareRTCSession::OnAddTrack(
...
void ShareRTCSession::OnRemoveTrack(
    webrtc::scoped_refptr<webrtc::RtpReceiverInterface> receiver) {
  RTC_LOG(LS_INFO) << __FUNCTION__ << " " << receiver->id();
  main_wnd_->QueueUIThreadCallback(TRACK_REMOVED, receiver->track().release());
}

void ShareRTCSession::OnIceCandidate(const webrtc::IceCandidate* candidate) {
...
void ShareRTCSession::OnSignedIn() {
  RTC_LOG(LS_INFO) << __FUNCTION__;
  main_wnd_->SwitchToPeerList(client_->peers());
}

void ShareRTCSession::OnDisconnected() {
  RTC_LOG(LS_INFO) << __FUNCTION__;
...
void ShareRTCSession::OnPeerConnected(int id, const std::string& name) {
  RTC_LOG(LS_INFO) << __FUNCTION__;
...
void ShareRTCSession::OnPeerDisconnected(int id) {
  RTC_LOG(LS_INFO) << __FUNCTION__;
  if (id == peer_id_) {
...
void ShareRTCSession::OnMessageFromPeer(int peer_id, const std::string& message) {
...
void ShareRTCSession::OnMessageSent(int err) {
  // Process the next pending message if any.
  main_wnd_->QueueUIThreadCallback(SEND_MESSAGE_TO_PEER, nullptr);
}

void ShareRTCSession::OnServerConnectionFailure() {
...
void ShareRTCSession::StartLogin(const std::string& server, int port) {
...
void ShareRTCSession::DisconnectFromServer() {
  if (client_->is_connected())
    client_->SignOut();
}

void ShareRTCSession::ConnectToPeer(int peer_id) {
  RTC_DCHECK(peer_id_ == -1);
...
void ShareRTCSession::AddTracks() {
...
void ShareRTCSession::DisconnectFromCurrentPeer() {
  RTC_LOG(LS_INFO) << __FUNCTION__;
  if (peer_connection_) {
...
void ShareRTCSession::UIThreadCallback(int msg_id, void* data) {
  switch (msg_id) {
...
void ShareRTCSession::OnSuccess(webrtc::SessionDescriptionInterface* desc) {
...
void ShareRTCSession::OnFailure(webrtc::RTCError error) {
  RTC_LOG(LS_ERROR) << ToString(error.type()) << ": " << error.message();
}

void ShareRTCSession::SendMessage(const std::string& json_object) {
